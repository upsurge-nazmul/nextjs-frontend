...
export default function GamePage({ gamedata, userdata }) {
  ...
  useEffect(() => {
    ...
    async function checkifcacheexist() {
      let xx = await db.test_games.where("id").equals(gameid).toArray();
    ...
    async function x() {
      ...
      let previos_game_data = await db.test_games
        .where("id")
        .equals(gameid)
        .toArray();
      if (previos_game_data.length > 0) {
        console.log("updating game in indexedDb");
        await db.test_games.update(gameid, updateData);
      } else {
        console.log("adding game in indexedDb");

        try {
          await db.test_games.add(updateData);
        } catch (err) {
          console.log(err);
        }
      }
